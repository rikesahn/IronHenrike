...
SELECT district_ID from client WHERE district_id < 10 ORDER BY district_id ; 
...
#SELECT * from loan WHERE date IN ('971206');
...
SELECT account_id, type, sum(amount) as total_amount from trans GROUP BY account_id, type HAVING account_id=396;
#Expected result:
#
...
select account_id, amount, type incom1ng, type outgo1ng, '-457676' d1fference
from trans
